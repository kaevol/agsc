...
def transform_traj_to_global_coordinate(traj: torch.Tensor, position: torch.Tensor,
    ...
    traj = torch.matmul(rotation_matrix.unsqueeze(-1)).squeeze(-1)
    traj = traj + position.unsqueeze(-2)
    return traj
...
def compute_corner_points(center_position: torch.Tensor, heading: torch.Tensor, length: torch.Tensor,
                          width: torch.Tensor) -> torch.Tensor:
    """
    Compute four corner points of a vehicle given its center position, heading, length and width.

    Args:
        center_position: [*, 2] Center position of vehicles
        heading: [*] Heading angle in radians
        length: [*] Vehicle length
        width: [*] Vehicle width

    Returns:
        corner_points: [*, 4, 2] Four corner points (front-left, front-right, rear-right, rear-left)
    """
    # Half dimensions
    half_length = length / 2.0
    half_width = width / 2.0

    # Corner offsets in vehicle coordinate system
    ...
    rotation_matrix = generate_counterclockwise_rotation_matrix(heading)
    corners_global = torch.matmul(rotation_matrix.unsqueeze(-3), corners_local.unsqueeze(-1)).squeeze(-1)
    corners_global = corners_global + center_position.unsqueeze(-2)

    return corners_global


def compute_corner_offsets(center_position: torch.Tensor, corner_points: torch.Tensor) -> torch.Tensor:
    """
    Compute corner point offsets relative to center position.

    Args:
        center_position: [*, 2] Center position
        corner_points: [*, 4, 2] Four corner points

    Returns:
        offsets: [*, 4, 2] Offsets of corners relative to center
    """
    return corner_points - center_position.unsqueeze(-2)