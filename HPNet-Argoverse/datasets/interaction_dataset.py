...
class InteractionDataset(Dataset):
    def __init__(self,
        ...
        self._corner_type = ['front_left', 'front_right', 'rear_right', 'rear_left']
    ...
    def get_features(self, 
        ...
        corner_position = torch.zeros(num_agents, self.num_steps, 4, 2, dtype=torch.float)
        corner_offsets = torch.zeros(num_agents, self.num_steps, 4, 2, dtype=torch.float)
        ...
        for track_id, track_df in df.groupby('track_id'):
            ...
            # Compute corner points for visible steps
            for step_idx in agent_steps:
                corners = compute_corner_points(
                    agent_position[agent_idx, step_idx],
                    agent_heading[agent_idx, step_idx],
                    agent_length[agent_idx],
                    agent_width[agent_idx]
                )
                corner_position[agent_idx, step_idx] = corners
                corner_offsets[agent_idx, step_idx] = compute_corner_offsets(
                    agent_position[agent_idx, step_idx],
                    corners
                )
        ...
        # Store corner data
        data['corner']['num_nodes'] = num_agents * 4  # 4 corners per agent
        data['corner']['position'] = corner_position.reshape(-1, self.num_steps, 2)  # [(N1,...,Nb)*4, T, 2]
        data['corner']['offsets'] = corner_offsets.reshape(-1, self.num_steps, 2)  # [(N1,...,Nb)*4, T, 2]
        data['corner']['visible_mask'] = visible_mask.unsqueeze(1).repeat(1, 4, 1).reshape(-1, self.num_steps)  # [(N1,...,Nb)*4, T]
        
        # Create corner to agent edge index
        corner_to_agent_edge_index = []
        for i in range(num_agents):
            for j in range(4):
                corner_idx = i * 4 + j
                corner_to_agent_edge_index.append([corner_idx, i])
        corner_to_agent_edge_index = torch.tensor(corner_to_agent_edge_index, dtype=torch.long).t()
        data['corner', 'agent']['corner_to_agent_edge_index'] = corner_to_agent_edge_index
        ...
        data['lane']['visible_mask'] = torch.ones(num_lanes, dtype=torch.bool)  # All lanes are visible
    ...
    def get(self, idx: int) -> HeteroData:
        ...
        # Check if lane visible_mask is missing (backward compatibility)
        if 'lane' in data_dict and 'visible_mask' not in data_dict['lane']:
            num_lanes = data_dict['lane']['num_nodes']
            data_dict['lane']['visible_mask'] = torch.ones(num_lanes, dtype=torch.bool)
        
        # Check if this is old processed data without corner information
        if 'corner' not in data_dict:
            # Compute corner data on the fly for backward compatibility
            num_agents = data_dict['agent']['num_nodes']
            num_steps = data_dict['agent']['position'].size(1)
            
            # Get device from existing data
            device = data_dict['agent']['position'].device
            
            # Initialize corner data
            corner_position = torch.zeros(num_agents, num_steps, 4, 2, dtype=torch.float, device=device)
            corner_offsets = torch.zeros(num_agents, num_steps, 4, 2, dtype=torch.float, device=device)
            
            # Compute corners for each visible timestep
            for agent_idx in range(num_agents):
                for step_idx in range(num_steps):
                    if data_dict['agent']['visible_mask'][agent_idx, step_idx]:
                        corners = compute_corner_points(
                            data_dict['agent']['position'][agent_idx, step_idx],
                            data_dict['agent']['heading'][agent_idx, step_idx],
                            data_dict['agent']['length'][agent_idx],
                            data_dict['agent']['width'][agent_idx]
                        )
                        corner_position[agent_idx, step_idx] = corners
                        corner_offsets[agent_idx, step_idx] = compute_corner_offsets(
                            data_dict['agent']['position'][agent_idx, step_idx],
            ...
            # Add corner data to dictionary
            data_dict['corner'] = {
                'num_nodes': num_agents * 4,
                'position': corner_position.reshape(-1, num_steps, 2),
                'offsets': corner_offsets.reshape(-1, num_steps, 2),
                'visible_mask': data_dict['agent']['visible_mask'].unsqueeze(1).repeat(1, 4, 1).reshape(-1, num_steps)
            }
            ...
            for i in range(num_agents):
                for j in range(4):
                    corner_idx = i * 4 + j
                    corner_to_agent_edge_index.append([corner_idx, i])
            data_dict['corner', 'agent'] = {
                'corner_to_agent_edge_index': torch.tensor(corner_to_agent_edge_index, dtype=torch.long, device=device).t()
            }
        
        return HeteroData(data_dict)