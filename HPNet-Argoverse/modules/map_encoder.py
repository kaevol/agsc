...
class MapEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        self.p2l_attn_layer = GraphAttention(hidden_dim=hidden_dim, num_heads=num_heads, dropout=dropout,
    ...
    def forward(self, data: Batch) -> torch.Tensor:
        ...
        p2l_edge_vector = transform_point_to_local_coordinate(p2l_position_p[p2l_edge_index[0]],
        ...
        p2l_edge_attr_input = torch.stack(
            [p2l_edge_attr_length, p2l_edge_attr_theta, p2l_edge_attr_heading, p2l_edge_attr_type], dim=-1)
        ...
        l2l_edge_attr_type.append(F.one_hot(torch.tensor(self._l2l_edge_type.index('left_neighbor')),
                                            num_classes=len(self._l2l_edge_type)).to(
            l2l_left_neighbor_edge_index.device).repeat(num_left_neighbor_edges, 1))
        ...
        l2l_edge_attr_type.append(F.one_hot(torch.tensor(self._l2l_edge_type.index('right_neighbor')),
                                            num_classes=len(self._l2l_edge_type)).to(
            l2l_right_neighbor_edge_index.device).repeat(num_right_neighbor_edges, 1))
        ...
        l2l_edge_attr_input = torch.cat(
            [l2l_edge_attr_length.unsqueeze(-1), l2l_edge_attr_theta.unsqueeze(-1), l2l_edge_attr_heading.unsqueeze(-1),
             l2l_edge_attr_hop.unsqueeze(-1), l2l_edge_attr_type.long()], dim=-1)
        l2l_edge_attr_embs = self.l2l_emb_layer(input=l2l_edge_attr_input)

        # attention
        # p2l
        l_embs = self.p2l_attn_layer(x=[p_embs, l_embs], edge_index=p2l_edge_index,
                                     edge_attr=p2l_edge_attr_embs)  # [(M1,...,Mb),D]
