...
class Backbone(nn.Module):
    ...
    def forward(self, data: Batch, l_embs: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        # Check if corner data is available
        has_corner_data = 'corner' in data and 'offsets' in data['corner']
        ...
        if has_corner_data:
            ...
            c2a_edge_attr_input = torch.stack([c2a_edge_attr_length, c2a_edge_attr_theta, c2a_edge_attr_type.float()], dim=-1)
            ...
            # No corner data available, just reshape a_embs
            pass  # a_embs already has the correct shape
