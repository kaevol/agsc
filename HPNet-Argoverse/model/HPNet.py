...
class HPNet(pl.LightningModule):
    ...
    def compute_corner_loss(self, pred_corners, target_corners, target_mask):
        ...
        if self.corner_loss_type == 'FDE':
            ...
            corner_loss = self.reg_loss(
                pred_corners[..., -1, :, :][target_mask[..., -1].unsqueeze(-1).expand(-1, 4, -1)],
                target_corners[..., -1, :, :][target_mask[..., -1].unsqueeze(-1).expand(-1, 4, -1)])
        else:  # ADE
            # Compute average loss over all frames
            mask_expanded = target_mask.unsqueeze(-2).expand(-1, -1, 4, -1)
            corner_loss = self.reg_loss(pred_corners[mask_expanded],
                                        target_corners[mask_expanded])
        return corner_loss
    ...
    def training_step(self, data, batch_idx):
        ...
        self.log('train_reg_loss_refine', reg_loss_refine, prog_bar=True, on_step=True, on_epoch=True, batch_size=1,
                 sync_dist=True)
        self.log('train_corner_loss_propose', corner_loss_propose, prog_bar=True, on_step=True, on_epoch=True,
                 batch_size=1,
                 sync_dist=True)
        self.log('train_corner_loss_refine', corner_loss_refine, prog_bar=True, on_step=True, on_epoch=True,
                 batch_size=1,
                 sync_dist=True)
        self.log('train_corner_loss_weight', corner_loss_weight, prog_bar=True, on_step=True, on_epoch=True,
                 batch_size=1,
                 sync_dist=True)
    ...
    def validation_step(self, data, batch_idx):
        ...
        self.log('val_reg_loss_refine', reg_loss_refine, prog_bar=True, on_step=False, on_epoch=True, batch_size=1,
                 sync_dist=True)
        self.log('val_corner_loss_propose', corner_loss_propose, prog_bar=True, on_step=False, on_epoch=True,
                 batch_size=1,
                 sync_dist=True)
        self.log('val_corner_loss_refine', corner_loss_refine, prog_bar=True, on_step=False, on_epoch=True,
                 batch_size=1,
                 sync_dist=True)
